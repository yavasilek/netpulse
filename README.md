...
- оценка задержки, джиттера и примерных потерь без тяжёлого speed test;
...
- профиль защиты с доверенной страной и оператором выходного узла;
- зелёный, жёлтый или красный статус соответствия защищённому маршруту;
- самодиагностика фоновой работы и экспорт локального отчёта;
- поиск и фильтры истории сетевых событий;
- безопасное обновление через GitHub Releases с видимым прогрессом загрузки.
...
- Качество оценивается тремя короткими TCP-подключениями, которые не измеряют
  пропускную способность и почти не расходуют трафик.
...
Для лёгкой оценки качества выполняются TCP-подключения к публичным DNS-узлам
Cloudflare и Google без отправки пользовательских данных.
